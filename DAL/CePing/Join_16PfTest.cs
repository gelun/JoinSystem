...
namespace DAL
{
	public class Join_16PfTest
    {
		...
		public static int Join_16PfTestAdd(Entity.Join_16PfTest info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(CommandType.StoredProcedure, "Join_16PfTestAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_16PfTestEdit(Entity.Join_16PfTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "Join_16PfTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_16PfTest] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_16PfTest Join_16PfTestEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_16PfTest] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "DELETE [Join_16PfTest]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
