...
namespace DAL
{
	public class Join_TestWL
    {
		...
		public static int Join_TestWLAdd(Entity.Join_TestWL info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(CommandType.StoredProcedure, "Join_TestWLAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_TestWLEdit(Entity.Join_TestWL info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "Join_TestWLEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_TestWL] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_TestWL Join_TestWLEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_TestWL] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "DELETE [Join_TestWL]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
