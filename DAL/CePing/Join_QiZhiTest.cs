...
namespace DAL
{
	public class Join_QiZhiTest
    {
		...
		public static int Join_QiZhiTestAdd(Entity.Join_QiZhiTest info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(CommandType.StoredProcedure, "Join_QiZhiTestAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_QiZhiTestEdit(Entity.Join_QiZhiTest info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "Join_QiZhiTestEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_QiZhiTest] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_QiZhiTest Join_QiZhiTestEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_QiZhiTest] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "DELETE [Join_QiZhiTest]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
