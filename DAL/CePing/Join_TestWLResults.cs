...
namespace DAL
{
	public class Join_TestWLResults
    {
		...
		public static int Join_TestWLResultsAdd(Entity.Join_TestWLResults info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(CommandType.StoredProcedure, "Join_TestWLResultsAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_TestWLResultsEdit(Entity.Join_TestWLResults info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "Join_TestWLResultsEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
        ...
        //{
        //    int intReturnValue = 0;
        //    intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "UPDATE [Join_TestWLResults] SET IsPause = "+  info.IsPause +"  WHERE WlId =  " +  info.WlId);
        //    if(intReturnValue == 1)
        //        return true;
        //    return false;
        //}
		...
		{
			return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_TestWLResults] WHERE WlId = "+WlId+";").Tables[0];
		}
		...
		public static Entity.Join_TestWLResults Join_TestWLResultsEntityGet(int WlId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_TestWLResults] WHERE WlId = "+WlId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "DELETE [Join_TestWLResults]  WHERE WlId =  " + WlId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
