...
namespace DAL
{
	public class Join_StudyAbroadAnswer
    {
		...
		public static int Join_StudyAbroadAnswerAdd(Entity.Join_StudyAbroadAnswer info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(CommandType.StoredProcedure, "Join_StudyAbroadAnswerAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_StudyAbroadAnswerEdit(Entity.Join_StudyAbroadAnswer info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "Join_StudyAbroadAnswerEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_StudyAbroadAnswer] WHERE AnswerId = "+AnswerId+";").Tables[0];
		}
		...
		public static Entity.Join_StudyAbroadAnswer Join_StudyAbroadAnswerEntityGet(int AnswerId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_StudyAbroadAnswer] WHERE AnswerId = "+AnswerId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "DELETE [Join_StudyAbroadAnswer]  WHERE AnswerId =  " + AnswerId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
