...
namespace DAL
{
	public class Join_StudyAbroad
    {
		...
		public static int Join_StudyAbroadAdd(Entity.Join_StudyAbroad info)
		{
			...
			return Basic.Utils.StrToInt(SqlDB.ExecuteScalar(CommandType.StoredProcedure, "Join_StudyAbroadAdd", prams).ToString(), -1);
		}
		...
		public static bool Join_StudyAbroadEdit(Entity.Join_StudyAbroad info)
		{
			...
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.StoredProcedure, "Join_StudyAbroadEdit", prams);
			if(intReturnValue == 1)
				return true;
			return false;
		}
		...
		{
			return SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_StudyAbroad] WHERE TestId = "+TestId+";").Tables[0];
		}
		...
		public static Entity.Join_StudyAbroad Join_StudyAbroadEntityGet(int TestId)
		{
			...
			DataTable dt = SqlDB.ExecuteDataset(CommandType.Text, "SELECT * FROM [Join_StudyAbroad] WHERE TestId = "+TestId+";").Tables[0];
			if(dt.Rows.Count >0)
			{
		...
		{
			int intReturnValue = 0;
			intReturnValue = SqlDB.ExecuteNonQuery(CommandType.Text, "DELETE [Join_StudyAbroad]  WHERE TestId =  " + TestId);
			if(intReturnValue == 1)
				return true;
			return false;
		}
